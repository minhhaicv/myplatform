...
class treeBehavior extends Model{
	...
	public function flat($root, $spacer = ' ', $display = 'title') {
	    ...
	    foreach($data as $key => $row) {
	        if (count($right)>0) {
	            ...
	            while ($right[count($right)-1]<$row['Category']['rght']) {
	                array_pop($right);
	            }
	        }
	        
	        // display indented node title
            $result[$key] = str_repeat($spacer,count($right)).$row['Category'][$display];
	             
	        $right[] = $row['Category']['rght'];
	    }
	...
	public function rebuild($parent, $left) {
	    ...
	    foreach($result as $row) {
	        $right = $this->rebuild($row['Category']['id'], $right);
	    }
	    
	    
	    $option = array(
            	       'fields' => array('lft' => $left, 'rght' => $right),
	                    'where' => "id = ".$parent
	    );
	...
	public function add($target, $update) {
	    $option = array (
	                    'select' => "{$this->alias}.id, {$this->alias}.rght",
	    ...
	    $update[$this->left] = $flag;
	    $update[$this->right] = $flag + 1;
